...
class AsteroidPage extends Component {
  ...
  componentDidMount() {
    fetch("http://127.0.0.1:8000/api/savedasteroid/?search=161%20Psyche")
      .then((res) => res.json())
      .then((data) => this.setState({ asteroiddetail: data }));
  }
  ...
  render() {
    return (
      <Card className="asteroid-page" style={{ width: "28rem" }} border="light">
        ...
        <Card.Body className="body">
          <div className="container">
            <Card.Title style={{ flexGrow: 1 }}>
              {this.state.asteroiddetail[0].name}
            </Card.Title>
        ...
        <p className="text">{this.state.asteroiddetail[0].note}</p>
        <div className="card-footer text-muted">
          {this.state.asteroiddetail[0].date_saved}
        </div>
      </Card>
    );
  }
